var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Component, ViewChild } from "@angular/core";
import { MatTableDataSource, MatPaginator, MatSort, MatDialog } from "@angular/material";
import { ToasterService } from "@services/toaster.service";
import { NgxSpinnerService } from "ngx-spinner";
import { ConfirmComponent } from "@shared/components/confirm/confirm.component";
import { AddsponsorComponent } from "./addsponsor/addsponsor.component";
import { ApiService } from '@services/api.service';
import { EEndpoints } from '@enums/endpoints';
import { ActivatedRoute } from "@angular/router";
var SponsorComponent = /** @class */ (function () {
    function SponsorComponent(toasterService, apiService, dialog, route, spinner) {
        this.toasterService = toasterService;
        this.apiService = apiService;
        this.dialog = dialog;
        this.route = route;
        this.spinner = spinner;
        this.displayedColumns = ["id", "name", "description", "edit", "delete"];
        this.perm = {};
        this.perm = this.route.snapshot.data;
    }
    SponsorComponent.prototype.ngOnInit = function () {
        this.spinner.show();
        this.getSponsors();
    };
    SponsorComponent.prototype.applyFilter = function (filterValue) {
        this.dataSource.filter = filterValue.trim().toLowerCase();
        if (this.dataSource.paginator) {
            this.dataSource.paginator.firstPage();
        }
    };
    SponsorComponent.prototype.getSponsors = function () {
        var _this = this;
        this.apiService.get(EEndpoints.sponsors).subscribe(function (data) {
            if (data.code == 100) {
                data.result.forEach(function (f) {
                    f.isActive = f.statusRecordId === 1 ? true : false;
                });
                _this.dataSource = new MatTableDataSource(data.result);
                _this.dataSource.paginator = _this.paginator;
                _this.dataSource.sort = _this.sort;
                _this.spinner.hide();
            }
            else {
                _this.spinner.hide();
                setTimeout(function () {
                    _this.toasterService.showToaster(data.message);
                }, 300);
            }
        }, function (err) { return console.error("Failed! " + err); });
    };
    SponsorComponent.prototype.addSponsor = function () {
        var _this = this;
        var action = "create";
        var dialogRef = this.dialog.open(AddsponsorComponent, {
            width: "800px",
            data: {
                action: action,
                id: 0
            }
        });
        dialogRef.afterClosed().subscribe(function (result) {
            _this.getSponsors();
        });
    };
    SponsorComponent.prototype.updateSponsor = function (id) {
        var _this = this;
        var action = "update";
        var dialogRef = this.dialog.open(AddsponsorComponent, {
            width: "700px",
            data: {
                action: action,
                id: id
            }
        });
        dialogRef.afterClosed().subscribe(function (result) {
            _this.getSponsors();
        });
    };
    SponsorComponent.prototype.confirmDelete = function (id) {
        var _this = this;
        var dialogRef = this.dialog.open(ConfirmComponent, {
            width: "400px",
            data: {
                text: "�Do you whant to delete this record" + id,
                action: '<i class="fas fa-trash fa-lg"></i> Delete'
            }
        });
        dialogRef.afterClosed().subscribe(function (result) {
            if (result !== undefined) {
                var confirm_1 = result.confirm;
                if (confirm_1)
                    _this.deleteArtistType(id);
            }
        });
    };
    SponsorComponent.prototype.deleteArtistType = function (id) {
        var _this = this;
        this.spinner.show();
        this.apiService.delete(EEndpoints.sponsor, { id: id }).subscribe(function (data) {
            if (data.code == 100) {
                _this.getSponsors();
                setTimeout(function () {
                    _this.toasterService.showToaster("This record has been deleted.");
                }, 300);
            }
            else {
                _this.spinner.hide();
                setTimeout(function () {
                    _this.toasterService.showToaster(data.message);
                }, 300);
            }
        }, function (err) { return console.error("Failed! " + err); });
    };
    __decorate([
        ViewChild(MatPaginator),
        __metadata("design:type", MatPaginator)
    ], SponsorComponent.prototype, "paginator", void 0);
    __decorate([
        ViewChild(MatSort),
        __metadata("design:type", MatSort)
    ], SponsorComponent.prototype, "sort", void 0);
    SponsorComponent = __decorate([
        Component({
            selector: "app-sponsor",
            templateUrl: "./sponsor.component.html"
        }),
        __metadata("design:paramtypes", [ToasterService,
            ApiService,
            MatDialog,
            ActivatedRoute,
            NgxSpinnerService])
    ], SponsorComponent);
    return SponsorComponent;
}());
export { SponsorComponent };
//# sourceMappingURL=sponsor.component.js.map